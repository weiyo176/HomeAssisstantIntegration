...
module.exports = {
    ...
    setTurnGate : async function (page, search) {
        let result = {"suc" : true};

        if (search.method == "get") {
            // get all turn gates info
            result["val"] = await page.evaluate( () => Array.from( document.getElementsByTagName("HOME-ASSISTANT")[0].renderRoot.children[0].renderRoot.children[0].children[1].children[0].renderRoot.children[0].renderRoot.children[0].children[1].children[0].children[0].renderRoot.children[0].children[1].children[0].renderRoot.children[0].children[1].children, element => element.children[0].renderRoot.children[0].renderRoot.children[1].innerText) );
        }
            ...
            await page.evaluate( () => Array.from( document.getElementsByTagName("HOME-ASSISTANT")[0].renderRoot.children[0].renderRoot.children[0].children[1].children[0].renderRoot.children[0].renderRoot.children[0].children[1].children[0].children[0].renderRoot.children[0].children[1].children[0].renderRoot.children[0].children[1].children, element => element.children[0].renderRoot.children[0].children[0].renderRoot.children[0].renderRoot.children[0].children[1].children[0].children[0].click()) );
	    if (result[0] != null) {
		result["suc"] = false;
		result["msg"] = "failed to turn the gate";
	    }	
        }
        return result;
    },

    setSensor : async function (page, search) {
        let result = {"suc" : true};
    ...
    crawlMapConsole : async function (url, search) {
        ...
        if (search.type == "turn_gate") {
            result = await this.setTurnGate(page, search);
        }
        else if (search.type == "sensor") {
            result = await this.setSensor(page, search);
        }
